...
import { formatFooter, parseFooterArgs } from "../extensions/footer.ts";
...
function main(): void {
  ...
  const footerText = formatFooter(
    {
      model: "smoke-test-model",
      contextWindow: 4096,
      categories: {
        system: { tokens: breakdown.systemPrompt.tokens },
        skills: { tokens: breakdown.skills.tokens },
        tools: { tokens: breakdown.tools.tokens },
        conversation: { tokens: 42 },
        free: { tokens: 1000 }
  ...
  if (!footerText.includes("System:")) throw new Error("footer smoke failed");
